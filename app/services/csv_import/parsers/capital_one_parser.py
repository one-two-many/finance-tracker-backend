...
class CapitalOneParser(CSVParser):
    """
    ...
    def get_required_headers(self) -> List[str]:
        return ["Transaction Date", "Description"]

    def detect(self, csv_content: str) -> float:
        """
        Detect Capital One format by checking for specific headers.
        """
        required_headers = set(self.get_required_headers())
        confidence = calculate_header_confidence(csv_content, required_headers)

        # Boost confidence if Capital One-specific headers are present
        try:
            csv_reader = csv.DictReader(StringIO(csv_content))
            headers = set(csv_reader.fieldnames or [])

            # Capital One specific headers
            capone_specific = {"Posted Date", "Card No.", "Category"}
            if capone_specific & headers:
                confidence = min(1.0, confidence + 0.2)

            # Uses Debit/Credit like Citi
    ...
    def parse(self, csv_content: str, account_type: Optional[str] = None) -> List[ParsedTransaction]:
        """
        Parse Capital One CSV into standardized transactions.
        ...
        if not required.issubset(headers):
            raise ValueError(
                f"Invalid Capital One CSV format. Expected headers: {required}. Found: {headers}"
            )
        ...
        for row in csv_reader:
            ...
            try:
                ...
                transaction_date = parse_date(date_str, "%Y-%m-%d")
